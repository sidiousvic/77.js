...
- Learned how to filter odd numbers in an array using a `.reduce()` function whose current value is multiplied by itself times the remainder of its half, `current * (current % 2)`. This will multiply odd values by 1 and even ones by 0, effectively subtracting them from the sum. `arr.reduce((a, b) => {a + b * (b % 2)}, 0);` **remember to set initialValue argument to 0!**
